...
class TestKeyValueStores(typeOfStore: String) {
  ...
  def testNullsWithSerde() {
    if (serde) {
      ...
      expect(classOf[NullPointerException], expectedNPEMessage) { store.get(null) }
      expect(classOf[NullPointerException], expectedNPEMessage) { store.delete(null) }
      expect(classOf[NullPointerException], expectedNPEMessage) { store.put(null, a) }
      expect(classOf[NullPointerException], expectedNPEMessage) { store.put(a, null) }
      expect(classOf[NullPointerException], expectedNPEMessage) { store.putAll(List(new Entry(a, a), new Entry[Array[Byte], Array[Byte]](a, null))) }
      expect(classOf[NullPointerException], expectedNPEMessage) { store.putAll(List(new Entry[Array[Byte], Array[Byte]](null, a))) }
      expect(classOf[NullPointerException], expectedNPEMessage) { store.range(a, null) }
      expect(classOf[NullPointerException], expectedNPEMessage) { store.range(null, a) }
    }
  }
