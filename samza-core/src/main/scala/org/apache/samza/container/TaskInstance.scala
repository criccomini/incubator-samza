...
class TaskInstance(
  ...
  def registerConsumers {
    ...
    inputStreams.foreach(systemStream => {
      ...
      val offset = offsetManager(systemStreamPartition)
      ...
      metrics.addOffsetGauge(systemStream, () => {
        offsetManager(systemStreamPartition)
          .getOrElse(throw new SamzaException("No offset defined for partition %s while trying to update metrics gauge. This is unexpected." format (systemStreamPartition)))
      })
    })
  }
