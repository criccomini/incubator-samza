...
public class MockSystemFactory implements SystemFactory {
  ...
  public static class MockSystemConsumerConfig {
    ...
    public int getPartitionsPerStream() {
      return config.getInt("system." + systemName + ".partitions.per.stream", DEFAULT_PARTITION_COUNT);
    }

    public int getMessagesPerBatch() {
      return config.getInt("system." + systemName + ".messages.per.batch", DEFAULT_MESSAGES_PER_BATCH);
    }

    public int getConsumerThreadCount() {
      return config.getInt("system." + systemName + ".consumer.thread.count", DEFAULT_CONSUMER_THREAD_COUNT);
    }
  }
}
